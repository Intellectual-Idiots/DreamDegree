    ...
    <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg p-6 hover:border-[var(--color-primary)] transition-all duration-300">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getDifficultyColor(degree.difficulty_level)}`}>
              {t(degree.difficulty_level.charAt(0).toUpperCase() + degree.difficulty_level.slice(1))}
            </span>
            <span className={`px-2 py-1 rounded text-xs font-bold ${getQualificationColor(degree.qualification_score)}`}>
              {t(`${degree.qualification_score}% Qualified`)}
            </span>
            {degree.total_gaps > 0 && (
              <span className="px-2 py-1 bg-orange-100 text-orange-700 rounded text-xs font-medium">
                {t(`${degree.total_gaps} gaps`)}
              </span>
            )}
          </div>
        ...
        <div className="text-right">
          ...
          {degree.aps_gap < 0 && (
            <div className="text-sm text-red-400">{t(`Need ${Math.abs(degree.aps_gap)} more`)}</div>
          )}
        </div>
      </div>
      ...
      <div className="mb-4">
        <h4 className="text-sm font-medium text-[var(--color-text)] mb-2">{t('Requirements Analysis:')}</h4>
        <div className="space-y-2">
          {/* APS Requirement */}
          <div className="flex justify-between items-center text-sm">
            <span>{t('APS Score:')}</span>
            <span className={degree.aps_gap >= 0 ? 'text-green-400' : 'text-red-400'}>
              {t(`Required: ${degree.aps} | Gap: ${degree.aps_gap >= 0 ? '+' : ''}${degree.aps_gap}`)}
            </span>
          </div>
          ...
          {Object.entries(degree.subject_gaps).map(([subject, gap]) => (
            <div key={subject} className="flex justify-between items-center text-sm">
              <span className="capitalize">{t(`${subject.replace('_', ' ')}:`)}</span>
              <span className={gap >= 0 ? 'text-green-400' : 'text-red-400'}>
                {t(`Gap: ${gap >= 0 ? '+' : ''}${gap} points`)}
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="mb-4">
        <h4 className="text-sm font-medium text-[var(--color-text)] mb-2">{t('Full Requirements:')}</h4>
        <div className="flex flex-wrap gap-2 text-sm">
          ...
          {degree["physical sciences"] && (
            <span className="bg-[var(--color-surface-muted)] px-2 py-1 rounded">{t(`Physical Sciences: ${degree["physical sciences"]}%+`)}</span>
          )}
          {degree["life sciences"] && (
            <span className="bg-[var(--color-surface-muted)] px-2 py-1 rounded">{t(`Life Sciences: ${degree["life sciences"]}%+`)}</span>
          )}
        </div>
      </div>
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[var(--color-page-bg)] flex items-center justify-center">
        <div className="text-[var(--color-text)] text-lg">{t('Loading gap analysis...')}</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[var(--color-page-bg)] text-[var(--color-text)]">
      ...
      <div className="bg-[var(--color-surface)] border-b border-[var(--color-border)] py-6">
        <div className="max-w-6xl mx-auto px-6">
          <h1 className="text-3xl font-bold text-[var(--color-text)] mb-2">{t('Gap Analysis')}</h1>
          <p className="text-[var(--color-text-subtle)] text-lg">
            {t("Degrees you almost qualify for - here's what you need to bridge the gap.")}
          </p>
        </div>
      </div>
        ...
        <div className="mb-8">
          <div className="flex flex-wrap gap-4">
            {Object.entries(categoryCounts).map(([difficulty, count]) => (
              ...
              >
                {t(difficulty === 'all' ? 'All Degrees' : difficulty.charAt(0).toUpperCase() + difficulty.slice(1))}
        ...
        {filteredDegrees.length > 0 ? (
          <>
            <div className="mb-6">
              <p className="text-[var(--color-text-subtle)]">
                {t(`Showing ${filteredDegrees.length} degrees you almost qualify for`)}
              </p>
            </div>
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-[var(--color-text)] mb-2">
              {t('No degrees in this difficulty category')}
            </h3>
            <p className="text-[var(--color-text-subtle)]">
              {t('Try exploring other difficulty levels to find degrees that might be a good fit.')}
            </p>
          </div>
        )}
      </div>
